...
TEST_CASE("util::Xorshift128Plus parameters & basic usage", BE_CATCH_TAGS) {
   ...
   rnd::xs128p prng;
   rnd::xs128p::state_type state { 16490336266968443936ull, 16834447057089888969ull };
...
TEST_CASE("util::Xorshift128Plus seed sequences", BE_CATCH_TAGS) {
   ...
   SECTION("DummySSeq") {
      ...
      rnd::xs128p::state_type state { 0x1234567812345678ULL, 0x1234567812345678ULL };
...
TEST_CASE("util::Xorshift128Plus seed from generator", BE_CATCH_TAGS) {
   SECTION("U32 generator") {
      ...
      rnd::xs128p::state_type state { 0x8b8bff008b8bff00ULL, 0x8b8bff008b8bff00ULL };
      prng.seed(dgen);
      REQUIRE(prng.state() == state);
   }
   ...
   SECTION("U64 generator") {
      ...
      Xorshift128Plus<> prng;
      rnd::xs128p::state_type state { 0x8b8bff0012344321ULL, 0x8b8bff0012344321ULL };
      prng.seed(dgen);
      REQUIRE(prng.state() == state);
   }
}

...
int interpolate_string(lua_State* L) {
   if (lua_type(L, 1) == LUA_TSTRING) { // anything else is returned unmodified
      ...
      if (lua_type(L, 2) == LUA_TFUNCTION) {
         S output = util::interpolate_string(input, [L](SV str) {
            lua_pushvalue(L, 2); // $ input fn fn
            lua_pushlstring(L, str.data(), str.length()); // $ input fn fn key
         ...
         S output = util::interpolate_string(input, [L](SV str) {
            lua_pushlstring(L, str.data(), str.length()); // $ input table key
